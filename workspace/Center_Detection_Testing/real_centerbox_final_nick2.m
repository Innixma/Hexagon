function [centerSize, numSides, wallAngles] = real_centerbox_final_nick2(CropImg)
    % Main code by Danesh, 
...
[apks, alocs, ~, ap] = findpeaks(augmented_dists);
...
for i = 1:length(possible_indices)-1
    if possible_indices(i+1) == possible_indices(i) + 1
        ...
        elseif ap(possible_indices(i+1)) < ap(possible_indices(i))
